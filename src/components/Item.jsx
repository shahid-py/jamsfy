...
const Item = ({ item }) => {
  return (
    <div
      key={item.id}
      class="w-full bg-white px-3   flex flex-col justify-center items-center  relative z-[5]"
    >
      <LazyLoadImage
        alt={item.title}
        src={item.image}
      ...
      <div>
        ...
        <h6 class="text-3xl font-bold text-gray-900 dark:text-white">{`$: ${item.price}`}</h6>
      </div>
    </div>
  );
};
